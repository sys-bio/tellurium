"""
    ...
    auto-generated code (2016-03-02T16:49:52)
    sedmlDoc: L1V2  
    workingDir: /tmp/tmpJI2NNe_sedml/_te_testcase_05
...
for k in range(plot_0_0_0.shape[1]):
    if k == 0:
        plt.plot(plot_0_0_0[:,k], plot_0_2_1[:,k], '-o', color='g', linewidth=1.5, markersize=4.0, alpha=0.8, label='task1.S1/task1.S2')
    else:
...
for k in range(plot_1_0_0.shape[1]):
    if k == 0:
        plt.plot(plot_1_0_0[:,k], plot_1_0_1[:,k], '-o', color='r', linewidth=1.5, markersize=4.0, alpha=0.8, label='task1.S2/max(task1.S2)')
    else:
