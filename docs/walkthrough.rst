...
* Start the Tellurium notebook app. Under the ``Language`` menu, select ``Find Kernels...``. A pop with a ``Scan`` button should appear. Click the ``Scan`` button. The results of the scan show all the kernels available to Tellurium. The built-in Python 3 and Node.js kernels are always available. Additional kernels appear based on available Jupyter kernels.
