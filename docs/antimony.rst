...
Antimony (so named because the chemical symbol of the element is â€˜Sbâ€™)
was designed as a successor to Jarnacâ€™s model definition language, with
...
-  Package and re-use models as modules with defined or implied
   interfaces.
-  Create â€˜DNA strandâ€™ elements, which can pass reaction rates to
...
The 2.12 release added the ability to save extra â€˜annotation-likeâ€™
elements from the â€˜distributionsâ€™ SBML package, and fixed numerous bugs
in cvterm/SBOterm setting.

The 2.11 release quashed all known memory leaks, and added the ability
to define synthesis reactions with no id (i.e.Â â€™ -> S1; k1â€™)
...
The 2.8.0 release of Antimony included support for the SBML â€˜Flux
Balance Constraintsâ€™ package (version 1), as well as constraints in
general.

In the 2.7.1 release of Antimony, species marked
â€˜hasOnlySubstanceUnits=trueâ€™ in SBML now have a corresponding definition
in Antimony: â€˜species substanceOnly S1, S2â€™.

In the 2.7 release of Antimony, QTAntimony got several new improvements,
including displayed line numbers, find/replace, and a â€˜go to lineâ€™
option. A few new syntaxes were also added, including the ability to
concisely define elements plus their assignment rules (â€˜species S1 in C
:= 3+pâ€™), and to define submodules with implied parameters (â€˜A:
mod1(1,2)â€™).
...
translated to SBML using the â€˜distributionsâ€™ package, as well as using
custom annotations
...
Antimonyâ€™s â€˜flatteningâ€™ routine (which exports plain SBML) matches
libSBMLâ€™s flattening routine.
...
In the 2.1 version of Antimony, the â€˜importâ€™ handling became much more
...
as an SDK. Hierarchical models are exported using CellMLâ€™s hierarchy,
translated to accommodate their â€˜black boxâ€™ requirements.
...
called â€˜reactionsâ€™ and the elements are called â€˜speciesâ€™, but any set of
...
In subsequent examples in this tutorial, weâ€™ll be using this syntax to
...
in which case youâ€™ll also need to define its initial starting value. The
...
For more advanced usage of events, see `Antimonyâ€™s reference
documentation on
...
basic level, you define a re-usable module with the â€˜modelâ€™ syntax,
followed by parentheses where you define the elements you wish to
expose, then import it by using the modelâ€™s name, and the local
...
through the use of the â€˜importâ€™ keyword. At any point in the file
outside of a module definition, use the word â€˜importâ€™ followed by the
...
not used or defined as a species is â€˜undefinedâ€™; â€˜undefinedâ€™ symbols may
later be declared or used as species or as â€˜formulasâ€™, Antimonyâ€™s term
for constants and packaged equations like SBMLâ€™s assignment rules. In
...
different model is known as a â€˜moduleâ€™, and is minimally defined by
putting the keyword â€˜modelâ€™ (or â€˜moduleâ€™, if you like) and the name you
want to give the module at the beginning of the model definitions you
wish to encapsulate, and putting the keyword â€˜endâ€™ at the end:
...
This is usually not very helpful in and of itself-youâ€™ll likely want to
...
symbols in the submodule by name, prepended with the name youâ€™ve given
...
When combining multiple submodules, you can also â€˜attachâ€™ them to each
...
``A.S is B.S``. For example, letâ€™s say that we have a species which is
...
exactly the same reaction kinetics and initial concentrations. Letâ€™s now
...
irreversibly, and where the complex it forms degrades. Weâ€™ll need a new
...
definitions in a defined model is optional, all â€˜bareâ€™ declarations are
...
delay, and â€˜timeâ€™), use the ``timeconv`` keyword when declaring the
submodel:
...
Some models have â€˜boundary speciesâ€™ in their reactions, or species whose
concentrations do not change as a result of participating in a reaction.
To declare that a species is a boundary species, use the â€˜constâ€™
keyword:
...
While youâ€™re declaring it, you may want to be more specific by using the
â€˜speciesâ€™ keyword:
...
â€˜speciesâ€™ is not required. If, however, you have a species which never
appears in a reaction, you will need to use the â€˜speciesâ€™ keyword.
...
explicitly with the â€˜varâ€™ keyword:
...
prepending a â€˜$â€™ in front of it:

::

   S1 + $E -> ES;

This would set the level of â€˜Eâ€™ to be constant. You can use this symbol
...
start of the simulation. In SBML terms, they use the â€˜Initial
Assignmentâ€™ values. If the formula is to vary during the course of the
...
model definition language like Antimony, â€˜pHâ€™, â€˜k1â€™, â€˜k2â€™, and even the
formula â€˜-log10(pH)â€™ are static symbols that are being defined by
...
â€˜-log10(pH)â€™; the same string it will receive for k2. A request for the
mathematical expression for pH will receive the string â€œ8.2â€, since
thatâ€™s the last definition found in the file. As such, k1 and k2 will
...
when using submodules, as weâ€™ll get to in a bit). So for now, the above
...
compartment with the imaginative name â€˜default_compartmentâ€™ with a
constant volume of 1. You may define other compartments by using the
â€˜compartmentâ€™ keyword:
...
The volume of a compartment may be set with an â€˜=â€™ in the same manner as
...
To declare that something is in a compartment, the â€˜inâ€™ keyword is used,
...
Here are Antimonyâ€™s rules for determining which compartment something is
in:
...
-  If not, if the symbol is a member of a submodule with a declared
   ...
   compartment.
-  If not, the symbol is in the compartment â€˜default_compartmentâ€™, and
   is treated as having no declared compartment for the purposes of
   determining the compartments of other symbols.

Note that declaring that one compartment is â€˜inâ€™ a second compartment
...
definitions of one or more symbols at the moment when certain conditions
apply. The condition is expressed as a boolean formula, and the
definition changes are expressed as assignments, using the keyword â€˜atâ€™
...
(you may also claim an event is â€˜inâ€™ a compartment just like everything
else (â€˜E1 in comp1:â€™). This declaration will never change the
...
this can be encoded by using the â€˜afterâ€™ keyword:
...
change this, the keyword â€˜fromTriggerâ€™ is used:

::

   E1: at 2*z/y after (x>5), fromTrigger=false: y=3, x=r+2;

You may also declare â€˜fromTrigger=trueâ€™ to explicitly declare what is
the default.
...
trigger condition is â€˜trueâ€™, it should be considered to have just
...
the event. You may explicitly state the default by using â€˜t0 = trueâ€™.
...
in Antimony is formulated with a â€˜:=â€™ as:

::

   Ptot := P1 + P2 + PE;

In this example, â€˜Ptotâ€™ will continually be updated to reflect the total
amount of â€˜Pâ€™ present in the model.
...
defined to be â€˜boundary speciesâ€™ and thus â€˜constâ€™. Antimony doesnâ€™t have
a separate syntax for boundary species whose concentrations never change
vs.Â boundary species whose concentrations change due to assignment rules
...
that occurs at time = 20 with a magnitude of f.Â A trigger is used to set
...
Rate rules define the change in a symbolâ€™s value over time instead of
...
â€˜idâ€™ of an element, which must be unique to the model and which must
conform to certain naming conventions, and the â€˜nameâ€™ of an element,
...
everywhere. However, you may also set the â€˜display nameâ€™ of an element
by using the â€˜isâ€™ keyword and putting the name in quotes:
...
Adding an â€˜sâ€™ to the end of a unit name to make it plural is fine when
defining a unit: ``3600 second`` is the same as ``3600 seconds``.
Compound units may be created by using formulas with ``*``, ``/``, and
``^``. However, you must use base units when doing so (â€˜base unitsâ€™
...
Antimony does not calculate any derived units: in the above example, â€˜xâ€™
...
â€˜x = 10 mlâ€™ will create a parameter x and a unit â€˜mlâ€™.
...
we use ellipses ``...`` as shorthand for â€˜the formula for the element
upstream of meâ€™. Letâ€™s add a ribosome binding site that increases the
...
(genes), and other DNA strands. Letâ€™s wrap our model so far in a
...
default or if it contains ellipses explicitly (â€˜â€¦â€™), it will be equal to
...
The reaction rate for the production of â€˜prot1â€™ will be equal to
...
of. For example, if you set the reaction rate of G1 above to â€œk1*S1â€,
and include it in two strands, the net reaction rate will be
``k1*S1 + k1*S1``.

The purpose of prepending or postfixing a â€˜--â€™ to a strand is to declare
that the strand in question is designed to have DNA attached to it at
that end. If exactly one DNA strand is defined with an upstream â€˜--â€™ in
...
and visa versa with a defined downstream â€˜--â€™ in its definition:
...
The module â€˜longâ€™ will have two strands: ``P3â€“A.P1â€“A.RBS1â€“A.G1`` and
``A.P2â€“A.RBS2â€“A.G2â€“G3``.

Submodule strands intended to be used in the middle of other strands
should be defined with â€˜--â€™ both upstream and downstream of the strand
in question:
...
If multiple strands are defined with upstream or downstream â€œâ€“â€ marks,
...
added to the SBML â€˜listOfModifiersâ€™ for the reaction in question.
...
This effectively defines S3 to have the equation
``k1*s1^2 + k2*s1 + k3``.

In addition, there are several built-in functions defined in Antimony.
All of the functions present in the MathML subset used in SBML Level 3
...
The â€˜truncatedâ€™ forms of all functions allow one to define inclusive
...
(i.e.Â ``A.mean=2.4``; ``A.confidenceInterval={S1, 8.2}``);
...
these constraints and objective functions is known as the â€˜Flux Balance
Constraintsâ€™ package. As of v2.8.0 of Antimony, these constraints can
...
In this example, the file â€˜models1.txtâ€™ is an Antimony file that defines
the module â€˜mod1â€™, and the file â€˜oscli.xmlâ€™ is an SBML file that defines
a model named â€˜oscliâ€™. The Antimony module â€˜mod2â€™ may then use modules
...
imported files will all contribute to the default â€˜\__mainâ€™ module. Most
...
By default, libantimony will examine the â€˜importâ€™ text to determine
...
The library will attempt to load â€˜antimony/import/file2.txtâ€™ instead of
looking for â€˜file2.txtâ€™ directly. For creating files in-memory or when
...
fileâ€™s actual location. The second entry must be exactly as it appears
in the first fileâ€™s â€˜importâ€™ directive, between the quotation marks.

Importing and Exporting Antimony Models
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Once you have created an Antimony file, you can convert it to SBML or
CellML using â€˜sbtranslateâ€™ or the â€˜QTAntimonyâ€™ visual editor (both
...
model, including the overall â€˜\__mainâ€™ module (if it contains anything).
...
â€˜Viewâ€™ menu, you can turn on or off the SBML and CellML tabs. Select the
tabs to translate and view the working model in those different formats.

The SBML tabs can additionally be configured to use the â€˜Hierarchical
Model Compositionâ€™ package constructs. Select â€˜Edit/Flatten SBML tab(s)â€™
or hit control-F to toggle between this version and the old â€˜flattenedâ€™
...
â€˜compâ€™ package enabled, and to select â€˜WITH_COMP_SBMLâ€™ from the CMake
menu.)
...
â€˜sbtranslateâ€™ executable. Instructions for use are available by running
...
format is given with the â€˜-oâ€™ flag: ``-o antimony``, ``-o sbml``,
``-o cellml``, or ``-o sbml-comp`` (the last to output files with the
SBML â€˜compâ€™ package constructs).
...
â€œ[prefix].xmlâ€, where [prefix] is the original filename with â€˜.txtâ€™
removed (if present).
...
will output two files in the working directory: â€˜oscli.txtâ€™ and
â€˜ffn.txtâ€™ (in the antimony format).

.. code:: bash

   sbtranslate model1.txt -o sbml-comp

will output â€˜model1.xmlâ€™ in the working directory, containing all models
in the â€˜model1.txtâ€™ file, using the SBML â€˜compâ€™ package.
...
-  Boundary (â€˜constâ€™) species in Antimony are set boundary=true and
   constant=false in SBML.
-  Variable (â€˜varâ€™) species in Antimony are set boundary=false and
   constant=false in SBML.
...
-  Any symbol with the MathML csymbol â€˜timeâ€™ in SBML becomes â€˜timeâ€™ in
   Antimony.
-  Any formula with the symbol â€˜timeâ€™ in it in Antimony will become the
   MathML csymbol â€˜timeâ€™ in in SBML.
-  The MathML csymbol â€˜delayâ€™ in SBML disappears in Antimony.
-  Any SBML version 2 level 1 function with the MathML csymbol â€˜timeâ€™ in
   it will become a local variable with the name â€˜time_refâ€™ in Antimony.
   This â€˜time_refâ€™ is added to the functionâ€™s interface (as the last in
   the list of symbols), and any uses of the function are modified to
   use â€˜timeâ€™ in the call. In other words, a function â€œfunction(x, y):
   x+y*timeâ€ becomes â€œfunction(x, y, time_ref): x + y*time_refâ€, and
   formulas that use â€œfunction(A, B)â€ become â€œfunction(A, B, time)â€
-  A variety of Antimony keywords, if found in SBML models as IDs, are
   renamed to add an appended â€˜\_â€™. So the ID ``compartment`` becomes
