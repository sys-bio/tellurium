...
class SEDMLCodeFactory(object):
    ...
    def outputPlot2DToPython(doc, output):
        ...
        for kc, curve in enumerate(output.getListOfCurves()):
            ...
            lines.append("    if k == 0:")
            lines.append("        plt.plot({}[:,k], {}[:,k], color='{}', linewidth={}, markersize={}, alpha={}, label='{}')".format(xId, yId, color, settings.linewidth, settings.markersize, settings.alpha, yLabel))
            lines.append("    else:")
            lines.append("        plt.plot({}[:,k], {}[:,k], color='{}', linewidth={}, markersize={}, alpha={})".format(xId, yId, color, settings.linewidth, settings.markersize, settings.alpha))
    ...
    def outputPlot3DToPython(doc, output):
        ...
        for kc, surf in enumerate(output.getListOfSurfaces()):
            ...
            lines.append("for k in range({}.shape[1]):".format(xId))
            lines.append("    if k == 0:")
            lines.append("        ax.plot({}[:,k], {}[:,k], {}[:,k], color='{}', linewidth=1.5, markersize=4.0, alpha=0.8, label='{}')".format(xId, yId, zId, color, zLabel))
            lines.append("    else:")
            lines.append("        ax.plot({}[:,k], {}[:,k], {}[:,k], color='{}', linewidth=1.5, markersize=4.0, alpha=0.8)".format(xId, yId, zId, color))
