"""
    ...
    auto-generated code (2016-03-07T10:13:04)
    sedmlDoc: L1V1  
...
for k in range(time.shape[1]):
    if k == 0:
        plt.plot(time[:,k], tim1[:,k], '-o', color='r', linewidth=1.5, markersize=3.0, alpha=0.8, label='tim mRNA (total)')
    else:
        plt.plot(time[:,k], tim1[:,k], '-o', color='r', linewidth=1.5, markersize=3.0, alpha=0.8)
for k in range(time.shape[1]):
    if k == 0:
        plt.plot(time[:,k], tim2[:,k], '-o', color='b', linewidth=1.5, markersize=3.0, alpha=0.8, label='tim mRNA (changed parameters)')
    else:
        plt.plot(time[:,k], tim2[:,k], '-o', color='b', linewidth=1.5, markersize=3.0, alpha=0.8)
for k in range(time.shape[1]):
    if k == 0:
        plt.plot(time[:,k], tim3[:,k], '-o', color='g', linewidth=1.5, markersize=3.0, alpha=0.8, label='tim mRNA (difference v1-v2+20)')
    else:
