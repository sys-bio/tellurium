...
workingDir = r'/home/mkoenig/git/tellurium/tellurium/tests/testdata/sedml/sedx/_te_sedMLBIOM21'
...
for k in range(time.shape[1]):
    if k == 0:
        plt.plot(time[:,k], tim1[:,k], marker = '.', color='r', linewidth=1.5, markersize=3.0, alpha=0.8, label='tim mRNA (total)')
    else:
        plt.plot(time[:,k], tim1[:,k], marker = '.', color='r', linewidth=1.5, markersize=3.0, alpha=0.8)
for k in range(time.shape[1]):
    if k == 0:
        plt.plot(time[:,k], tim2[:,k], marker = '.', color='b', linewidth=1.5, markersize=3.0, alpha=0.8, label='tim mRNA (changed parameters)')
    else:
        plt.plot(time[:,k], tim2[:,k], marker = '.', color='b', linewidth=1.5, markersize=3.0, alpha=0.8)
for k in range(time.shape[1]):
    if k == 0:
        plt.plot(time[:,k], tim3[:,k], marker = '.', color='g', linewidth=1.5, markersize=3.0, alpha=0.8, label='tim mRNA (difference v1-v2+20)')
    else:
        plt.plot(time[:,k], tim3[:,k], marker = '.', color='g', linewidth=1.5, markersize=3.0, alpha=0.8)
