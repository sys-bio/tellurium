"""
    ...
    auto-generated code (2016-03-07T10:12:49)
    sedmlDoc: L1V1  
...
# DataGenerator <tim1>
__var__tim1_v1_0 = np.transpose(np.array([sim['[Mt]'] for sim in task1]))
if len(__var__tim1_v1_0.shape) == 1:
     __var__tim1_v1_0.shape += (1,)
tim1 = __var__tim1_v1_0

# DataGenerator <tim2>
__var__tim2_v2_0 = np.transpose(np.array([sim['[Mt]'] for sim in task2]))
if len(__var__tim2_v2_0.shape) == 1:
     __var__tim2_v2_0.shape += (1,)
tim2 = __var__tim2_v2_0
...
for k in range(time.shape[1]):
    if k == 0:
        plt.plot(time[:,k], tim1[:,k], '-o', color='r', linewidth=1.5, markersize=3.0, alpha=0.8, label='tim1')
    else:
        plt.plot(time[:,k], tim1[:,k], '-o', color='r', linewidth=1.5, markersize=3.0, alpha=0.8)
for k in range(time.shape[1]):
    if k == 0:
        plt.plot(time[:,k], tim2[:,k], '-o', color='b', linewidth=1.5, markersize=3.0, alpha=0.8, label='tim2')
    else:
        plt.plot(time[:,k], tim2[:,k], '-o', color='b', linewidth=1.5, markersize=3.0, alpha=0.8)
for k in range(time.shape[1]):
    if k == 0:
        plt.plot(time[:,k], tim3[:,k], '-o', color='g', linewidth=1.5, markersize=3.0, alpha=0.8, label='tim3')
    else:
        plt.plot(time[:,k], tim3[:,k], '-o', color='g', linewidth=1.5, markersize=3.0, alpha=0.8)
